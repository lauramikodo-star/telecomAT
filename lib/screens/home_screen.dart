...
class AppState extends ChangeNotifier {
  ...
  Future<void> login(String nd, String password) async {
    loading = true;
    notifyListeners();
    try {
      _loginResult = await api.login(nd, password);
    } catch (e) {
      _loginResult = {'error': e.toString()};
    } finally {
      loading = false;
      notifyListeners();
    }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: const TabBar(
            tabs: [
              Tab(text: 'Recharge', icon: Icon(Icons.payment)),
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            if (state.line4gInfo != null) _4gInfoCard(data: state.line4gInfo!),
          ],
        ),
      ),
    );
